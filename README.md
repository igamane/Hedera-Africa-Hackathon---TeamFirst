...
## Roadmap Ideas
- Replace mock club data with CMS/Mongo-backed collections and image uploads.
- Upgrade the real-time layer to a true WebSocket (e.g., Hedera consensus mirror + server push).
- Expand analytics with Hedera Mirror Node queries for lifetime totals, retention, and geographic breakdowns.
- Add role-based auth so verified club admins can withdraw funds, edit profiles, and publish updates.
